...
class SetAccessPointStaticallyTest {
    ...
    fun normalTest() = runBlocking {
        ...
        device.wlanControlCommandV2?.setAccessPointStatically(
            ssid = testData.ssid,
            ...
            subnetMask = testData.subnetMask ?: "",
            defaultGateway = testData.defaultGateway ?: "",
            proxy = testData.proxy?.let { Proxy(it) },
        )
        withTimeout(100) {
            deferred.await()
        }
        assertTrue(true, "setAccessPointStatically")
    }

    /**
     * Not connected exception for setAccessPointStatically call.
     */
    @Test
    fun exceptionApiTest() = runBlocking {
        ...
        try {
            ...
            device.disconnect()
            wlanControlCommandV2?.setAccessPointStatically(
                ssid = testData.ssid,
                ssidStealth = testData.ssidStealth ?: true,
                security = testData.security?.let { WifiSecurityMode.getFromValue(it) }
                    ?: WifiSecurityMode.NONE,
    ...
    /**
     * Write exception for setAccessPointStatically call.
     */
    @Test
