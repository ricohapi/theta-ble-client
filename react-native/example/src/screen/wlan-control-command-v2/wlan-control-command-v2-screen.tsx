...
> = ({ navigation }) => {
  ...
  const setNetworkTypeNotify = async (callback?: (value?: NetworkTypeEnum, error?: NotifyError) => void) => {
    ...
    try {
      ...
      const newMessage = `setNetworkTypeNotify: ${callback != null ? 'enabled' : 'disabled'}`;
  ...
  const setConnectedWifiInfoNotify = async (callback?: (value?: ConnectedWifiInfo, error?: NotifyError) => void) => {
    ...
    try {
      ...
      const newMessage = `setConnectedWifiInfoNotify: ${callback != null ? 'enabled' : 'disabled'}`;
      console.log(newMessage);
      setMessage(newMessage);
    } catch (error) {
      setMessage(JSON.stringify(error, null, 2));
  ...
  const onSetConnectedWifiInfoNotify = async () => {
    setConnectedWifiInfoNotify((info) => {
      const newMessage = `Notify ConnectedWifiInfo: ${JSON.stringify(info, null, 2)}`;
      addMessage(newMessage);
      messageScrollToBottom();
    });
  };
