...
export class CameraStatusCommand extends ThetaService {
   ...
   */
  async setBatteryLevelNotify(
    callback?: (value?: number, error?: NotifyError) => void
  ) {
    try {
      await ThetaBleClient.nativeSetBatteryLevelNotify(
        this.device.id,
        callback ? true : false
      );
   ...
   * @param callback Notification function
   */
  async setBatteryStatusNotify(
    callback?: (value?: ChargingStateEnum, error?: NotifyError) => void
  ) {
    try {
      await ThetaBleClient.nativeSetBatteryStatusNotify(
        this.device.id,
        callback ? true : false
      );
      this.device.notifyList.set(
   ...
   * @param callback Notification function
   */
  async setCameraPowerNotify(
    callback?: (value?: CameraPowerEnum, error?: NotifyError) => void
  ) {
    try {
      await ThetaBleClient.nativeSetCameraPowerNotify(
        this.device.id,
        callback ? true : false
      );
      this.device.notifyList.set(
   ...
   * @param callback Notification function
   */
  async setCommandErrorDescriptionNotify(
    callback?: (
      value?: CommandErrorDescriptionEnum,
      error?: NotifyError
    ) => void
  ) {
    try {
      await ThetaBleClient.nativeSetCommandErrorDescriptionNotify(
        this.device.id,
        callback ? true : false
      );
      this.device.notifyList.set(
        ...
        callback
          ? (event: CommandErrorDescriptionNotify) => {
   ...
   * @param callback Notification function
   */
  async setPluginControlNotify(
    callback?: (value?: PluginControl, error?: NotifyError) => void
  ) {
    try {
      await ThetaBleClient.nativeSetPluginControlNotify(
        this.device.id,
        callback ? true : false
      );
      this.device.notifyList.set(
